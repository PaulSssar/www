...
class OrderListView(APIView):
    @swagger_auto_schema(
        ...
        manual_parameters=[openapi.Parameter('city', openapi.IN_QUERY, description="Город для поиска заказов",
                                             type=openapi.TYPE_STRING)],
...
class UserOrderListView(APIView):
    @swagger_auto_schema(
        ...
        manual_parameters=[
            openapi.Parameter('user_id', openapi.IN_QUERY, description="ID пользователя для поиска заказов",
                              type=openapi.TYPE_INTEGER)],
        responses={200: OrderSerializer(many=True)}
    )
    def get(self, request):
